...
def bulk_progress_report_pdf(request):
    ...
    for enrollment in enrollments:
        ...
        c.drawCentredString(width / 2, y, "MADEENATHU CM OLAVANNA")
        y -= 0.7 * cm
...
def acknowledgement_certificate_pdf(request, pk):
    ...
    p1 = f"This is to certify that <b>{student.full_name}</b> (Student ID: <b>{student.student_id}</b>) is a bonafide student of Madeenathu CM Olavanna."
    ...
    c.drawRightString(width - 2.5 * cm, y, "Madeenathu CM Olavanna")
...
@role_required(['admin', 'teacher'])
def cumulative_attendance_pdf(request, grade_id, division_id):
    """
    ...
    # Build PDF Response
    response = HttpResponse(content_type='application/pdf')
    clean_grade_name = grade.name.replace(' ', '_')
    clean_div_name = division.name if division else 'NoDiv'
    filename = f"cumulative_attendance_{clean_grade_name}_{clean_div_name}.pdf"
    response['Content-Disposition'] = f'attachment; filename="{filename}"'
    ...
    # Title & Subtitles
    elements.append(Paragraph("MADEENATHU CM OLAVANNA", inst_style))
...
def pwa_manifest(request):
    ...
    manifest_data = {
        "name": "Madeenathu CM Olavanna",
        "short_name": "Madeenathu CM Olavanna",
        "description": "Madeenathu CM Olavanna ERP & Student Portal",
        "start_url": "/home/",
        "display": "standalone",
...
def pwa_serviceworker(request):
    ...
    sw_code = """
const CACHE_NAME = 'madeenathu-cm-pwa-v4';
const urlsToCache = [
